...
| `MI_REGION`                | `ru`         | Регион Mi Fitness (`ru`, `de`, `us`, `cn`) |
| `TELEGRAM_ALLOWED_IDS`     | (empty)      | Список ID через запятую для доступа      |
| `IS_PUBLIC`                | `false`      | Сделать бота публичным для всех          |

### Первый запуск (привязка)
По умолчанию бот привязывается к **первому** пользователю, который напишет ему `/start`. Его Telegram ID сохраняется в файл `data/allowed_user.id`. Все данные (Xiaomi токены, база здоровья) будут привязаны именно к этому человеку.

Если вы хотите дать доступ друзьям, используйте `TELEGRAM_ALLOWED_IDS` или `IS_PUBLIC`.
