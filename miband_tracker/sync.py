    ...
    *,
    ...
    try:
        with sqlite_conn(db_path, row_factory=False) as conn:
            cursor = conn.cursor()
            from mi_fitness.const import get_health_api_base

            region = settings.mi_region
            base_url = get_health_api_base(region)
            async with MiHealthClient.from_token(
                str(token_path), base_url=base_url, region_tag=region
            ) as client:
